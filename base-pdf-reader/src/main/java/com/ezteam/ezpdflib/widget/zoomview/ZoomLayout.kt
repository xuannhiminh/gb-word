package com.ezteam.ezpdflib.widget.zoomview
...
open class ZoomLayout private constructor(
    ...
    var onlyAllow2Fingers = false


    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {

        if (onlyAllow2Fingers) {
            if (ev.pointerCount == 2 && ev.actionMasked == MotionEvent.ACTION_MOVE) {
                // two fingers sliding → intercept so we can pan the zoomed content
                return true
            }
            return false
        }
        ...
        engine.onInterceptTouchEvent(ev)

        if (ev.pointerCount >= 2) {
            // Pinch gesture: intercept
            return true
        }

        val zoom = engine.realZoom
        when (ev.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                val dx = ev.x - lastDownX
                val dy = ev.y - lastDownY

                if (zoom <= 1f) {
                    // Not zoomed: don't intercept scroll — let RecyclerView handle it
                    return false
                }
                ...
                if (absDx > absDy) {
                    // Horizontal pan: let ZoomEngine handle it
                    return true
                }

                // Vertical pan: check if content has room to scroll vertically
                val viewH = height.toFloat()
                val contentH = engine.contentHeight * zoom
                val panY = engine.scaledPanY

                val atTop = panY >= 0f && dy > 0f
                val atBottom = panY <= viewH - contentH && dy < 0f

                return false // Intercept only if there's room to scroll vertically inside zoomed content
            }
...
//        // Only consume the touch if we actually intercepted,
//        // otherwise let the RecyclerView scroll.
//        return engine.onTouchEvent(ev)
//    }
